...
import { ActionButton, BodyModal, ButtonModal, DescriptionModal, TitleModal } from 'src/pages/Setting/Networks/style';
...
const LoadMoreDropdown = (props: Props) => {
  const { stateWallet, networkId, passwordHash, explorer } = props;
  const { account, chainId, connectedSites, wallets } = stateWallet;
      ...
      <LoadMoreContent>
        {isMobile && (
          <LoadMoreOption onClick={openNewTab}>
            <DivImage>
              <Image src={images.wallet.expandView} alt="copy-gray" size={16} width={16} />
            </DivImage>
        ...
        <LoadMoreOption
          onClick={() => {
            ...
            (refDropdownLoadMore as any).current.hideDropdownContent();
          }}
        >
          <DivImage>
            <Image src={images.wallet.connectedSites} alt="copy-gray" size={16} width={16} />
          </DivImage>
          <DivChild marginLeft="20px">Connected sites</DivChild>
        </LoadMoreOption>
        <LoadMoreOption onClick={() => window.open(explorer, '_blank')}>
          <DivImage>
            <Image src={images.wallet.viewExplorer} alt="copy-gray" size={16} width={16} />
          </DivImage>
        ...
        >
          <DivImage>
            <Image src={images.wallet.exportKey} alt="copy-gray" size={16} width={16} />
          </DivImage>
        ...
        >
          <DivImage>
            <Image src={images.wallet.removeWallet} alt="copy-gray" size={16} width={16} />
          </DivImage>
  ...
  const renderConnectedSites = (sites) =>
    sites.map((site) => (
      <ConnectedSitesItem key={site} justifyContent="space-between">
        <DivFlex>
          <Image src={images.wallet.directionBlack} alt="trash" size={16} width={16} />
  ...
  return (
    <Div>
      ...
      {isShowExportPrivateKey && (
        <ModalExportPrivateKey isOpen={isShowExportPrivateKey} title="Export Private Key" onCloseModal={() => setShowExportPrivateKey(false)} />
      )}
      {isShowRemoveConfirmConnectedSite && (
        <ModalCustom isOpen={isShowRemoveConfirmConnectedSite} showCloseIcon={false}>
          <BodyModal>
            ...
            <ActionButton>
              <ButtonModal background="#ffffff" border="1px solid #461A57" color="#461A57" onClick={() => setShowRemoveConfirmConnectedSite(false)}>
                Cancel
              </ButtonModal>
              <ButtonModal background="#461A57" color="#ffffff" onClick={() => onRemoveConnectedSite(siteRemove)}>
                Disconnect
              </ButtonModal>
            </ActionButton>
          </BodyModal>
